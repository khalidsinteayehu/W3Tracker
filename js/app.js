...
function renderProjects() { const container = document.getElementById('projectList'); container.innerHTML = ''; state.projects.forEach(project => { const div = document.createElement('div'); div.className = 'project-item'; div.innerHTML = <div class="project-header"> <span>${project.name}</span> <span class="timestamp">${new Date(project.created).toLocaleDateString()}</span> </div> <div class="task-count">${project.tasks.length} tasks</div>; container.appendChild(div); }); }
